...
class GNFA(fa.FA):
    ...
    def to_regex(self, pos=None) -> str:
        """
        ...
        pos, G = self.to_latex(ripped_states=ripped_states, new_transitions=new_transitions)
        print(G)
        ...
        while len(new_states) > 2:
            ...
            ripped_states.append(q_rip)

            print(f'\\section{{Rip {q_rip}}}')
            pos, G = self.to_latex(ripped_states=ripped_states, new_transitions=new_transitions)
            print(G)
    ...
    def to_latex(self, pos=None, ripped_states=[], new_transitions={}) -> str:
        def convert_automaton_to_graph():
            E = [(i,j) for (i,j,label) in list(self.iter_transitions())]
            G = nx.DiGraph(E)
            return G
        def add_node_at_pos(name: str, i: float, j: float, is_start=False, is_final=False, fill_color=''):
            start_str, final_str, fill_str = '', '', ''
            if is_start:
                start_str = ',initial'
            if is_final:
                final_str = ',accepting'
            ...
            return f'\\node[state{start_str}{final_str}{fill_str}] ({name}) at ({i}, {j}) {{{name}}};'
        
        def tex_string(s: str):
            return f'${s}$'
        
        def add_transition(i: str, j: str, label: str, bend=False):
            bend_str, loop_str = '', ''
            if bend:
                bend_str = 'bend right'
            if i == j:
                loop_str = 'loop above'
            inner_str = f'[{bend_str},{loop_str}]'
            return f'\\path[->] ({i}) {inner_str} edge node{{{label}}}({j});' 

        def create_automaton(s):
            return '''\\begin{tikzpicture}[node distance=2cm]
                %s
            \\end{tikzpicture}''' % s

        if not pos:
            G = convert_automaton_to_graph()
            pos = nx.bfs_layout(G, self.initial_state)
        inner = []
        for node in self.states - set(ripped_states):
            is_start = node == self.initial_state
            is_final = node in self.final_states
            i, j = map(lambda x: x*5, pos[node])
            inner.append(add_node_at_pos(str(node), i, j, is_start=is_start, is_final=is_final))
        ...
        # print(transitions)
        for begin_state, the_dict in transitions.items():
            if begin_state in ripped_states:
                continue
            # group states based on char
            inv = {}
            ...
            # print(transitions)
            # print(f'Inv={inv}')
            for regex, to_states in inv.items():
                # print(f"######### Regex={regex}, To_state={to_states} ###########")
                if regex is None:
                    continue
                if regex == '':
                    regex = '\\varepsilon'
                ...
                for to_state in to_states:
                    if to_state in ripped_states:
                        continue
                    back_trans = False
                    if to_state in transitions:
                        back_trans = begin_state in transitions[to_state] and begin_state != to_state
                    # new_transition_label = ', '.join(sorted(map(str,)))
                    new_transition_label = tex_string(regex)
                    # print(to_state, new_transition_label)
                    inner.append(add_transition(begin_state, to_state, new_transition_label, back_trans))
        
        return pos, create_automaton('\n'.join(inner))

if __name__ == '__main__':
    A = dfa.DFA.from_substring({"a","b","c"}, "abc")
    # print(A)
    G = GNFA.from_dfa(A)
    pos, automaton = G.to_latex()
    # print(automaton)
    R = G.to_regex(pos)
    # print(R)
    # print(latex_automaton(A, modified_states={4}))